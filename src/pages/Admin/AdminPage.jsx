...
const TinyBarChart = ({ values }) => {
    ...
    return <div className={styles.barChart}>{values.map((v, i) => <span key={`${i}-${v}`} style={{ height: `${Math.max((v / max) * 100, 14)}%` }} />)}</div>;
};
...
function AdminPage() {
    ...
    const paymentBreakdown = ['COD', 'CARD', 'WALLET', 'BANK'].map((mode) => orders.filter((item) => item.paymentMethod === mode).length);
    const orderStatusBreakdown = ['PENDING', 'CONFIRMED', 'PROCESSING', 'SHIPPING', 'COMPLETED', 'CANCELLED'].map((st) => orders.filter((item) => item.orderStatus === st).length);
    const activityBreakdown = ['LOGIN', 'VIEW_PRODUCT', 'VIEW_PRODUCT_LIST', 'PAYMENT'].map((ac) => activities.filter((item) => item.action === ac).length);
