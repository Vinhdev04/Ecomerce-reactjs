...
const paymentModes = [
    { id: 'COD', shortLabel: 'COD' },
    { id: 'CARD', shortLabel: 'CARD' },
    { id: 'WALLET', shortLabel: 'WALLET' },
    { id: 'BANK', shortLabel: 'BANK' }
];
...
const TinyLineChart = ({ values }) => {
    ...
    const line = values
        .map((value, index) => `${index * stepX},${height - (value / max) * 125 - 18}`)
        .join(' ');
...
function AdminPage() {
    ...
    const analytics = useMemo(() => {
        ...
        return {
            ...
            lowStockCount: products.filter((item) => {
                const stock = Number(item.stock || 0);
                return stock > 0 && stock <= 10;
            }).length,
            monthlySeries: [14, 19, 17, 25, 22, 31].map((base, index) => base + orders.length + index * 2),
            paymentBreakdown: paymentModes.map((option) => ({
                ...option,
                total: orders.filter((item) => item.paymentMethod === option.id).length
            })),
            recentActivity: [...orders, ...products, ...news, ...users]
    ...
    const overviewCards = useMemo(() => {
        ...
        const productCategories = new Set(products.map((item) => item.category).filter(Boolean)).size;
        const newsCategories = new Set(news.map((item) => item.category).filter(Boolean)).size;
        const averagePrice = products.length ? products.reduce((sum, item) => sum + Number(item.price || 0), 0) / products.length : 0;
        const averageReadTime = news.length ? Math.round(news.reduce((sum, item) => sum + Number(item.readTime || 0), 0) / news.length) : 0;

        const cardsBySection = {
            dashboard: [
                { label: 'Giá trị kho', value: money(analytics.inventoryValue), helper: 'giá trị tồn kho đang quản lý' },
                { label: 'Low stock', value: analytics.lowStockCount, helper: 'sản phẩm cần nhập thêm' },
                { label: 'Tin mới', value: news.length, helper: latestNews ? latestNews.title : 'chưa có bài viết' }
            ],
            payments: [
                { label: 'Đã thanh toán', value: orders.filter((item) => item.paymentStatus === 'PAID').length, helper: 'đơn đã ghi nhận tiền' },
                { label: 'Chờ xử lý', value: analytics.pendingPayments, helper: 'giao dịch cần xác minh' },
                { label: 'Online', value: orders.filter((item) => item.paymentMethod !== 'COD').length, helper: 'thẻ, ví điện tử, ngân hàng' }
            ],
            users: [
                { label: 'Admin', value: adminCount, helper: 'tài khoản có quyền quản trị' },
                { label: 'Customers', value: users.length - adminCount, helper: 'khách hàng đã đăng ký' },
                { label: 'Mới 7 ngày', value: users.filter((item) => Date.now() - new Date(item.createdAt).getTime() < 7 * 24 * 60 * 60 * 1000).length, helper: 'tài khoản tạo gần đây' }
            ],
            products: [
                { label: 'Danh mục', value: productCategories, helper: 'nhóm sản phẩm đang kinh doanh' },
                { label: 'Tồn kho thấp', value: analytics.lowStockCount, helper: 'sản phẩm dưới ngưỡng cảnh báo' },
                { label: 'Giá TB', value: money(averagePrice), helper: 'mức giá trung bình catalog' }
            ],
            news: [
                { label: 'Danh mục', value: newsCategories, helper: 'chuyên mục editorial đang dùng' },
                { label: 'Đọc trung bình', value: `${averageReadTime} phút`, helper: 'thời lượng đọc mỗi bài' },
                { label: 'Bài mới nhất', value: latestNews?.author || 'Chưa có', helper: latestNews ? latestNews.title : 'chưa có bài viết' }
            ]
        };

        return cardsBySection[activeSection] || cardsBySection.dashboard;
    }, [activeSection, analytics.inventoryValue, analytics.lowStockCount, analytics.pendingPayments, news, orders, products, users]);
    ...
    const gate = !userInfo
        ...
        : userInfo.role !== 'ADMIN'
          ? {
                ...
                desc: 'Tài khoản hiện tại chưa được cấp role ADMIN.',
                href: '/'
            }
          : null;
    ...
    const onNewsSubmit = async (event) => {
        event.preventDefault();
        setSubmitting(true);
        try {
            editingNewsId
        ...
        <div className={styles.adminShell}>
            ...
            <main className={styles.main}>
                <header className={styles.topbar}>
                    ...
                    <div className={styles.topbarActions}><div className={styles.searchBox}><input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Tìm trong khu quản trị..." /></div><button type="button" className={styles.secondaryBtn} onClick={() => fetchAdminData({ silent: true })}><FaArrowsRotate />{refreshing ? 'Đang sync' : 'Làm mới'}</button><button type="button" className={styles.primaryBtn} onClick={() => navigate('/')}>Xem storefront <FaChevronRight /></button></div>
                </header>
                ...
                <section className={styles.overviewGrid}>
                    {overviewCards.map((item, index) => <article key={item.label} className={styles.overviewCard} style={{ animationDelay: `${index * 70}ms` }}><span>{item.label}</span><strong>{item.value}</strong><small>{item.helper}</small></article>)}
                </section>

                {activeSection === 'dashboard' && <section className={styles.dashboardGrid}><article className={styles.panelCard}><div className={styles.sectionHeader}><div><p className={styles.kicker}>Performance</p><h3>Tăng trưởng hệ thống</h3></div><span className={styles.livePill}>live</span></div><TinyLineChart values={analytics.monthlySeries} /><div className={styles.chartLegend}>{['T1', 'T2', 'T3', 'T4', 'T5', 'T6'].map((item) => <span key={item}>{item}</span>)}</div></article><article className={styles.panelCard}><div className={styles.sectionHeader}><div><p className={styles.kicker}>Payment mix</p><h3>Cơ cấu thanh toán</h3></div></div><TinyBarChart values={analytics.paymentBreakdown.map((item) => item.total)} /><div className={styles.breakdownList}>{analytics.paymentBreakdown.map((item) => <div key={item.id}><span>{item.shortLabel}</span><strong>{item.total}</strong></div>)}</div></article><article className={styles.panelCard}><div className={styles.sectionHeader}><div><p className={styles.kicker}>Realtime feed</p><h3>Biến động dữ liệu</h3></div></div><div className={styles.feedList}>{analytics.recentActivity.map((item) => <div key={item.id} className={styles.feedItem}><div><strong>{item.title || item.name || item.orderCode || item.customerEmail}</strong><span>{formatDateTime(item.updatedAt || item.createdAt)}</span></div><b>{item.paymentStatus || item.category || item.role || 'updated'}</b></div>)}</div></article></section>}

                {activeSection === 'payments' && <section className={styles.pageGrid}><article className={styles.panelCard}><div className={styles.sectionHeader}><div><p className={styles.kicker}>Payments</p><h3>Quản lý giao dịch</h3></div><span className={styles.livePill}>{filteredOrders.length} giao dịch</span></div><div className={styles.tableWrap}><table className={styles.dataTable}><thead><tr><th>Mã đơn</th><th>Khách hàng</th><th>PTTT</th><th>Trạng thái</th><th>Tổng</th><th>Xem</th></tr></thead><tbody>{filteredOrders.map((item) => <tr key={item.id}><td><strong>{item.orderCode}</strong><span>{formatDateTime(item.createdAt)}</span></td><td><strong>{item.customerName}</strong><span>{item.customerEmail}</span></td><td><span className={styles.inlineTag}>{item.paymentMethod}</span></td><td><span className={`${styles.statusPill} ${styles[item.paymentStatus.toLowerCase()]}`}>{item.paymentStatus}</span></td><td>{money(item.total)}</td><td><button type="button" className={styles.iconBtn} onClick={() => setSelectedOrderId(item.id)}><FaEye /></button></td></tr>)}</tbody></table></div></article><article className={styles.panelCard}>{selectedOrder ? <><div className={styles.sectionHeader}><div><p className={styles.kicker}>Detail</p><h3>{selectedOrder.orderCode}</h3></div><span className={`${styles.statusPill} ${styles[selectedOrder.paymentStatus.toLowerCase()]}`}>{selectedOrder.paymentStatus}</span></div><div className={styles.detailCard}><div className={styles.detailHero}><div className={styles.detailBlock}><strong>{selectedOrder.customerName}</strong><span>{selectedOrder.customerPhone}</span><span>{selectedOrder.customerEmail}</span><span>{selectedOrder.shippingAddress}</span></div><div className={styles.detailStat}><span>Tổng thanh toán</span><strong>{money(selectedOrder.total)}</strong><small>{selectedOrder.items?.length || 0} sản phẩm trong đơn</small></div></div><div className={styles.statusGrid}><label>Payment<select value={selectedOrder.paymentStatus} onChange={(event) => onUpdateOrder(selectedOrder.id, { paymentStatus: event.target.value })}>{paymentStatuses.map((status) => <option key={status} value={status}>{status}</option>)}</select></label><label>Order<select value={selectedOrder.orderStatus} onChange={(event) => onUpdateOrder(selectedOrder.id, { orderStatus: event.target.value })}>{orderStatuses.map((status) => <option key={status} value={status}>{status}</option>)}</select></label></div><div className={styles.summaryList}><div><span>Tạm tính</span><strong>{money(selectedOrder.subtotal)}</strong></div><div><span>Ship</span><strong>{money(selectedOrder.shippingFee)}</strong></div><div><span>Phí thanh toán</span><strong>{money(selectedOrder.paymentFee)}</strong></div><div><span>Tổng cộng</span><strong>{money(selectedOrder.total)}</strong></div></div><div className={styles.orderItems}>{selectedOrder.items?.map((item, index) => <div key={`${item.id}-${index}`} className={styles.orderItem}><div><strong>{item.title}</strong><span>{item.quantity} x {money(item.price)}</span></div><b>{money(item.quantity * item.price)}</b></div>)}</div></div></> : <div className={styles.emptyBox}>Chưa có đơn hàng nào.</div>}</article></section>}

                {activeSection === 'users' && <section className={styles.pageGrid}><article className={styles.panelCard}><div className={styles.sectionHeader}><div><p className={styles.kicker}>User CRUD</p><h3>{editingUserId ? 'Cập nhật tài khoản' : 'Tạo tài khoản mới'}</h3></div><button type="button" className={styles.secondaryBtn} onClick={resetUser}><FaPlus />Mới</button></div><form className={styles.formGrid} onSubmit={onUserSubmit}><input value={userForm.name} onChange={(event) => setUserForm((prev) => ({ ...prev, name: event.target.value }))} placeholder="Tên hiển thị" /><input value={userForm.email} onChange={(event) => setUserForm((prev) => ({ ...prev, email: event.target.value }))} placeholder="Email" /><input type="password" value={userForm.password} onChange={(event) => setUserForm((prev) => ({ ...prev, password: event.target.value }))} placeholder={editingUserId ? 'Mật khẩu mới' : 'Mật khẩu'} /><select value={userForm.role} onChange={(event) => setUserForm((prev) => ({ ...prev, role: event.target.value }))}><option value="CUSTOMER">Customer</option><option value="ADMIN">Admin</option></select><button type="submit" className={styles.primaryBtn} disabled={submitting}>{submitting ? 'Đang lưu...' : editingUserId ? 'Cập nhật user' : 'Tạo user'}</button></form></article><article className={styles.panelCard}><div className={styles.sectionHeader}><div><p className={styles.kicker}>Directory</p><h3>Bảng tài khoản</h3></div></div><div className={styles.tableWrap}><table className={styles.dataTable}><thead><tr><th>Tên</th><th>Email</th><th>Role</th><th>Cập nhật</th><th>Thao tác</th></tr></thead><tbody>{filteredUsers.map((item) => <tr key={item.id}><td><strong>{item.name || item.email}</strong></td><td>{item.email}</td><td><span className={`${styles.statusPill} ${item.role === 'ADMIN' ? styles.admin : styles.neutral}`}>{item.role}</span></td><td>{formatDateTime(item.updatedAt || item.createdAt)}</td><td>{actions(() => { setEditingUserId(item.id); setUserForm({ name: item.name || '', email: item.email || '', password: '', role: item.role || 'CUSTOMER' }); }, () => onDelete('users', item.id))}</td></tr>)}</tbody></table></div></article></section>}

                {activeSection === 'products' && <section className={styles.pageGrid}><article className={styles.panelCard}><div className={styles.sectionHeader}><div><p className={styles.kicker}>Product studio</p><h3>{editingProductId ? 'Cập nhật sản phẩm' : 'Tạo sản phẩm mới'}</h3></div><button type="button" className={styles.secondaryBtn} onClick={resetProduct}><FaPlus />Mới</button></div><form className={styles.formGrid} onSubmit={onProductSubmit}><input value={productForm.title} onChange={(event) => setProductForm((prev) => ({ ...prev, title: event.target.value }))} placeholder="Tên sản phẩm" /><input value={productForm.category} onChange={(event) => setProductForm((prev) => ({ ...prev, category: event.target.value }))} placeholder="Danh mục" /><input type="number" value={productForm.price} onChange={(event) => setProductForm((prev) => ({ ...prev, price: event.target.value }))} placeholder="Giá" /><input type="number" value={productForm.stock} onChange={(event) => setProductForm((prev) => ({ ...prev, stock: event.target.value }))} placeholder="Tồn kho" /><input value={productForm.badge} onChange={(event) => setProductForm((prev) => ({ ...prev, badge: event.target.value }))} placeholder="Badge" /><input type="number" step="0.1" value={productForm.rating} onChange={(event) => setProductForm((prev) => ({ ...prev, rating: event.target.value }))} placeholder="Rating" /><input className={styles.span2} value={productForm.image} onChange={(event) => setProductForm((prev) => ({ ...prev, image: event.target.value }))} placeholder="Ảnh, ngăn cách dấu phẩy" /><input className={styles.span2} value={productForm.size} onChange={(event) => setProductForm((prev) => ({ ...prev, size: event.target.value }))} placeholder="Size, ngăn cách dấu phẩy" /><textarea className={styles.span2} rows="5" value={productForm.description} onChange={(event) => setProductForm((prev) => ({ ...prev, description: event.target.value }))} placeholder="Mô tả sản phẩm" /><button type="submit" className={styles.primaryBtn} disabled={submitting}>{submitting ? 'Đang lưu...' : editingProductId ? 'Cập nhật sản phẩm' : 'Tạo sản phẩm'}</button></form></article><article className={styles.panelCard}><div className={styles.sectionHeader}><div><p className={styles.kicker}>Catalog</p><h3>Bảng sản phẩm</h3></div></div><div className={styles.tableWrap}><table className={styles.dataTable}><thead><tr><th>Sản phẩm</th><th>Danh mục</th><th>Kho</th><th>Giá</th><th>Thao tác</th></tr></thead><tbody>{filteredProducts.map((item) => <tr key={item.id}><td><div className={styles.tableMedia}><img src={Array.isArray(item.image) ? item.image[0] : item.image} alt={item.title} /><div><strong>{item.title}</strong><span>{item.badge || 'No badge'}</span></div></div></td><td>{item.category}</td><td>{item.stock}</td><td>{money(item.price)}</td><td>{actions(() => { setEditingProductId(item.id); setProductForm({ title: item.title || '', description: item.description || '', price: String(item.price ?? ''), category: item.category || '', stock: String(item.stock ?? ''), rating: String(item.rating ?? ''), badge: item.badge || '', image: Array.isArray(item.image) ? item.image.join(', ') : '', size: Array.isArray(item.size) ? item.size.join(', ') : '' }); }, () => onDelete('products', item.id))}</td></tr>)}</tbody></table></div></article></section>}

                {activeSection === 'news' && <section className={styles.pageGrid}><article className={styles.panelCard}><div className={styles.sectionHeader}><div><p className={styles.kicker}>Editorial</p><h3>{editingNewsId ? 'Cập nhật bài viết' : 'Tạo bài viết mới'}</h3></div><button type="button" className={styles.secondaryBtn} onClick={resetNews}><FaPlus />Mới</button></div><form className={styles.formGrid} onSubmit={onNewsSubmit}><input className={styles.span2} value={newsForm.title} onChange={(event) => setNewsForm((prev) => ({ ...prev, title: event.target.value }))} placeholder="Tiêu đề" /><input value={newsForm.author} onChange={(event) => setNewsForm((prev) => ({ ...prev, author: event.target.value }))} placeholder="Tác giả" /><input value={newsForm.category} onChange={(event) => setNewsForm((prev) => ({ ...prev, category: event.target.value }))} placeholder="Danh mục" /><input value={newsForm.tags} onChange={(event) => setNewsForm((prev) => ({ ...prev, tags: event.target.value }))} placeholder="Tags" /><input type="number" value={newsForm.readTime} onChange={(event) => setNewsForm((prev) => ({ ...prev, readTime: event.target.value }))} placeholder="Read time" /><input className={styles.span2} value={newsForm.image} onChange={(event) => setNewsForm((prev) => ({ ...prev, image: event.target.value }))} placeholder="Ảnh cover" /><textarea className={styles.span2} rows="3" value={newsForm.summary} onChange={(event) => setNewsForm((prev) => ({ ...prev, summary: event.target.value }))} placeholder="Tóm tắt" /><textarea className={styles.span2} rows="8" value={newsForm.content} onChange={(event) => setNewsForm((prev) => ({ ...prev, content: event.target.value }))} placeholder="Nội dung" /><button type="submit" className={styles.primaryBtn} disabled={submitting}>{submitting ? 'Đang lưu...' : editingNewsId ? 'Cập nhật bài viết' : 'Tạo bài viết'}</button></form></article><article className={styles.panelCard}><div className={styles.sectionHeader}><div><p className={styles.kicker}>Publishing</p><h3>Bảng bài viết</h3></div></div><div className={styles.tableWrap}><table className={styles.dataTable}><thead><tr><th>Bài viết</th><th>Tác giả</th><th>Danh mục</th><th>Read time</th><th>Thao tác</th></tr></thead><tbody>{filteredNews.map((item) => <tr key={item.id}><td><div className={styles.tableMedia}><img src={item.image} alt={item.title} /><div><strong>{item.title}</strong><span>{item.summary}</span></div></div></td><td>{item.author || 'Admin'}</td><td>{item.category || 'General'}</td><td>{item.readTime ? `${item.readTime} phút` : 'Chưa có'}</td><td>{actions(() => { setEditingNewsId(item.id); setNewsForm({ title: item.title || '', summary: item.summary || '', content: item.content || '', image: item.image || '', author: item.author || '', category: item.category || '', tags: Array.isArray(item.tags) ? item.tags.join(', ') : '', readTime: item.readTime ? String(item.readTime) : '' }); }, () => onDelete('news', item.id))}</td></tr>)}</tbody></table></div></article></section>}
            </main>
        </div>
    );
}
